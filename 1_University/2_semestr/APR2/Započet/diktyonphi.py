...
class Node:
    ...
    def edges(self) -> Iterator[Edge]:
        """Return an iterator over edges from this node to its neighbors."""
        edges = []
        for neighbor_id in self.neighbor_ids:
           edges.append(self.to(neighbor_id)) 
        return edges
...
class Graph:
    ...
    ## moje fce
    def get_neighbors(self, node_id: Hashable) -> Dict[Hashable, Dict[str, Any]]:
        """
        Get the neighbors of a node.

        :param node_id: ID of the node.
        :return: Dictionary of neighbors and their attributes.
        """
        return self._nodes[node_id]._neighbors if node_id in self._nodes else {}
    
    def izolovane_uzly(self):
        izolovane_uzly = set()
        for node in self._nodes.values():
            if node.out_degree == 0:
                izolovane_uzly.add(node)
        return izolovane_uzly


    def sirka(self, start:Hashable, goal:Hashable) -> list:
      if start not in self or goal not in self:
        raise ValueError("Start or goal node not in graph")

      prohledane = set()
      fronta = [start]
      poradi =[]
      prohledane.add(start)
      ...
      while len(fronta) > 0:
        ...
        if vrchol == goal:
          return poradi
        for neighbor in self.node(vrchol).neighbor_ids:
          if neighbor not in prohledane:
            fronta.append(neighbor)
            prohledane.add(neighbor)

      raise ValueError("Mezi nodes neni cesta")
...
if __name__ == "__main__":
    ...
    #print(g.all_nodes_and_neighbors())
    ...
    ## @property je bomba (místo getter a setter)